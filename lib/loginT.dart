...
import 'package:memoire/homeEN.dart';
...
class _LoginState extends State<LoginT> {
  ...
  Future<void> signIn() async {
    ...
    try {
    final response=await Supabase.instance.client.from('compte').select('id_compte').eq('username=',user).eq('profile','enseignant').eq('password',pass).maybeSingle();
      print(response);
      if (response!=null) {
        print('LogIn succecful');
        int iduser=response['id_compte'];
         final res=await Supabase.instance.client.from('enseignant').select('id_prof').eq('id_compte',iduser).maybeSingle();
         if (res!=null) {
          int id=res['id_prof'];
          ...
          title: 'Login Successfully!',
           ...
           Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => Homeen(iduser: id,),));
         }
      }
      else{
        QuickAlert.show(
    ...
    body: Container(margin: EdgeInsets.all(12),
      child: ListView(
        children: [
          ...
          Container(margin: EdgeInsets.only(left:5 ,top: 15),child: Image.asset("images/teacher.png",height: 220,width: 250,fit: BoxFit.fill,)),
          Padding(
            padding: const EdgeInsets.only(left: 15,top: 20),
            child: Text("ENSEIGNANT",textAlign: TextAlign.center,style: TextStyle(fontSize: 30,fontWeight: FontWeight.bold,fontFamily: "indian",color: Color.fromARGB(255, 71, 71, 71)),),
          ),
        ],),),
