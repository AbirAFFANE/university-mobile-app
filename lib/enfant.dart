...
class Enfant extends StatefulWidget {
  final int iduser;
  const Enfant({super.key, required this.iduser});

  @override
  State<Enfant> createState() => _HomeeState();
}

class _HomeeState extends State<Enfant> {
    List <dynamic>data=[];
  ...
  Widget build(BuildContext context){
    ...
    endDrawer: Drawer(backgroundColor: const Color.fromARGB(255, 231, 232, 231),child: ListView(children: [
      ...
      bottomNavigationBar: SafeArea(
        child: Container(height: 80,decoration: BoxDecoration(gradient: LinearGradient(
                  colors: [Colors.cyanAccent, Colors.teal ],
                  begin: Alignment.topCenter,
                  end: Alignment.bottomCenter,
                ),borderRadius: BorderRadius.circular(20)),margin: EdgeInsets.only(bottom: 25,left: 20,right: 20),
        child: BottomNavigationBar(
          selectedLabelStyle: TextStyle(fontSize: 20,fontFamily: "indian",fontWeight: FontWeight.bold),
          unselectedLabelStyle: TextStyle(fontSize: 22,fontFamily: "indian",fontWeight: FontWeight.bold),
      ...
      body: Container(child: ListView(children: [
        Container(height: 20,),
        Container(decoration: BoxDecoration(borderRadius: BorderRadius.circular(20),boxShadow: [BoxShadow(color: Colors.black,blurRadius: 5,)],border: Border.all(width: 1)),margin: EdgeInsets.only(right: 20,left: 20,),height: 170,child: ClipRRect(borderRadius: BorderRadius.circular(20),child: Image.asset("images/learn.jpg",fit: BoxFit.fill,)),), 
        Container(margin: EdgeInsets.only(top: 20,left: 10,right: 10),height: 310,child: GridView(
