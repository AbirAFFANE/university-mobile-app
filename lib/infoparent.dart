...
class Infoparent extends StatefulWidget {
  final int iduser;
  const Infoparent({super.key, required this.iduser});

  @override
  State<Infoparent> createState() => _InfoenState();
}

class _InfoenState extends State<Infoparent> {
  List <dynamic>data=[];
Future<void> getdata() async { 
    
    try {
        final response = 
          await Supabase.instance.client.from('parent').select().eq('id_parent', widget.iduser).maybeSingle(); 
          print("$response");
...
void initState(){
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(backgroundColor: const Color.fromARGB(255, 16, 80, 118),toolbarHeight: 34,foregroundColor: Colors.white),
          ...
          Container(
            decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: [const Color.fromARGB(255, 16, 80, 118), const Color.fromARGB(255, 33, 122, 205)],
            ...
            child: Column(
              children: [
                ...
                CircleAvatar(
                  ...
                  child: Icon(Icons.person, size: 40, color: const Color.fromARGB(255, 16, 80, 118)),
                ),
                SizedBox(height: 10),
                Text(
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          prefixIcon: Icon(icon, color: const Color.fromARGB(255, 16, 80, 118)),
