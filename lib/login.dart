...
import 'package:memoire/homeE.dart';
...
class _LoginState extends State<Login> {
  ...
  Future<void> signIn() async {
    ...
    try {
    final response=await Supabase.instance.client.from('compte').select('id_compte').eq('username=',user).eq('profile','etudiant').eq('password',pass).maybeSingle();
      print(response);
      if (response!=null) {
        
        print('LogIn succecful');
        int iduser=response['id_compte'];
         final res=await Supabase.instance.client.from('etudiant').select('id_etudiant').eq('id_compte',iduser).maybeSingle();
         if (res!=null) {
          int id=res['id_etudiant'];
          ...
          Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => Homee(iduser: id,),));
    ...
    body: Container(margin: EdgeInsets.all(12),
      child: ListView(
        children: [
          ...
          Container(margin: EdgeInsets.only(left:5 ,top: 15),child: Image.asset("images/student.png",height: 220,width: 250,fit: BoxFit.fill,)),
          Padding(
            padding: const EdgeInsets.only(left: 15,top: 20),
            child: Text("ETUDIANT",textAlign: TextAlign.center,style: TextStyle(fontSize: 30,fontWeight: FontWeight.bold,fontFamily: "indian",color: Color.fromARGB(255, 71, 71, 71)),),
          ),
        ],),),
